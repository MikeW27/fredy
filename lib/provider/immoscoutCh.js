 ...
 * @param {Object} o - Raw listing object
 ...
 * @param {string[]} blacklist - Blacklist terms
 * @returns {boolean} True if the listing is not blacklisted
 */
 ...
 *
 * Reads Bright Data credentials from global admin settings (`brightDataApiToken`,
 * `brightDataZone`) and an optional page cap (`immoscout24ChMaxPages`).
 * Called with `this` bound to {@link FredyPipelineExecutioner}.
 *
...
async function getListings(url) {
  ...
  const apiToken = settings.brightDataApiToken;
  const zone = settings.brightDataZone;
  const maxPages = settings.immoscout24ChMaxPages || DEFAULT_MAX_PAGES;
...
/** @type {import('../types/providerConfig.js').ProviderConfig} */
const config = {
  url: null,
  // immoscout24.ch has no public URL sort parameter - the URL is passed through unchanged.
 ...
 *
 * Returns a fresh object on every call instead of mutating module-level state.
 * Two jobs can be in flight at once and shared mutable state would let the
 * second job overwrite the first job's URL and blacklist mid-run.
 *
 * @param {{url: string, enabled?: boolean}} sourceConfig The job's entry for this provider.
 * @param {string[]} [blacklist] Terms to filter listings out by.
 * @returns {import('../types/providerConfig.js').ProviderConfig} A configuration usable by a single pipeline run.
 */
...
/** @type {import('../types/providerMeta.js').ProviderMetaInformation} */
